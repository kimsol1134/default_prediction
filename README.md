...
|----------|----------|------|
| 🟢 **안전** (Green) | < 3.42% | 정상 승인 |
| 🟡 **주의** (Yellow) | 3.42% ~ 8.91% | 추가 심사 권장 |
| 🔴 **위험** (Red) | ≥ 8.91% | 거절 또는 정밀 심사 필수 |
...
- 모델 사용하려면:
  1. Git LFS 설치: `git lfs install`
  2. 모델 다운로드: `git lfs pull`
  3. 또는 Part3 노트북에서 모델 재학습
...
### SHAP 계산 실패
**메시지:** "VotingClassifier는 SHAP TreeExplainer 미지원"

**해결:**
- Part3 v3 최종 모델 사용 시 SHAP 정상 작동
- VotingClassifier (레거시 모델) 사용 시 간소화된 SHAP 표시
